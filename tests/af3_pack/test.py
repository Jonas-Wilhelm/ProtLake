...
OUT_DIR = "tests_out/af3_pack/test_lake"
OUT_DIR_READ_TEST = "tests_out/af3_pack/out_read"
BATCH_SIZE = 100
...
def main():
    ...
    # ----------- reading -----------
    df = dt.to_pandas()
    os.makedirs(OUT_DIR_READ_TEST, exist_ok=True)
    ...
    for i in random_indices:
        ...
        cif.write(os.path.join(OUT_DIR_READ_TEST, f"{df.iloc[i]['name']}__{df.iloc[i]['seed']}__{df.iloc[i]['sample']}__from_db.cif"))
        ...
        if os.path.exists(af3_cif_path):
            shutil.copy(af3_cif_path, os.path.join(OUT_DIR_READ_TEST, f"{df.iloc[i]['name']}__{df.iloc[i]['seed']}__{df.iloc[i]['sample']}__from_af3.cif"))
        ...
        with open(os.path.join(OUT_DIR_READ_TEST, f"{df.iloc[i]['name']}__{df.iloc[i]['seed']}__{df.iloc[i]['sample']}_conf__from_db.json"), "w") as f:
